...
<head>
    <meta charset="UTF-8">
    <title>Kelola Perekonomian</title>
...
<nav class="navbar navbar-dark navbar-admin py-3">
    <div class="container">
        <span class="navbar-brand fw-bold"><i class="bi bi-graph-up-arrow me-2"></i> KELOLA PEREKONOMIAN</span>
...
<div class="container mt-5">
    ...
    <div class="row g-4 justify-content-center">
        @foreach([
            ['bumd', 'Pelayanan BUMD', 'droplet'],
            ['tpid', 'Pelayanan TPID', 'shield-check'],
            ['tp2d', 'Pelayanan TP2D', 'gear'],
            ['umkm', 'Pemberdayaan UMKM', 'people']
        ] as $item)
        <div class="col-md-3">
            <div class="card card-menu p-4 text-center">
                <div class="icon-circle"><i class="bi bi-{{ $item[2] }}"></i></div>
                <h5 class="fw-bold mb-3">{{ $item[1] }}</h5>
                <a href="{{ route('admin.perekonomian.edit', $item[0]) }}" class="btn btn-primary fw-bold w-100">Kelola</a>
            </div>
        </div>
        @endforeach
    </div>
