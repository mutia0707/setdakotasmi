...
<head>
    <meta charset="UTF-8">
    <title>Kelola Pusat Unduhan</title>
...
<nav class="navbar navbar-dark navbar-admin py-3">
    <div class="container">
        <span class="navbar-brand fw-bold"><i class="bi bi-file-earmark-arrow-down-fill me-2"></i> KELOLA PUSAT UNDUHAN</span>
...
<div class="container mt-5" style="max-width: 900px;">
    ...
    <div class="card border-0 shadow-sm rounded-4 p-4 mb-4">
        <h6 class="fw-bold mb-3"><i class="bi bi-cloud-upload me-2"></i> Upload Dokumen Baru</h6>
        <form action="{{ route('admin.download.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="row g-3">
                <div class="col-md-5">
                    <input type="text" name="judul" class="form-control" placeholder="Nama Dokumen" required>
                </div>
                <div class="col-md-5">
    ...
    <div class="card border-0 shadow-sm rounded-4 p-4">
        <h6 class="fw-bold mb-3">Daftar Dokumen</h6>
...
<tbody>
                ...
                <tr>
                    <td>{{ $index + 1 }}</td>
                    <td>{{ $f->judul }}</td>
                    {{-- PERBAIKAN: Menggunakan Carbon::parse untuk menangani jika created_at berupa string --}}
                    <td>{{ \Carbon\Carbon::parse($f->created_at)->format('d-m-Y') }}</td>
                    <td>
                        <form action="{{ route('admin.download.destroy', $f->id) }}" method="POST">
                            @csrf 
                            @method('DELETE')
